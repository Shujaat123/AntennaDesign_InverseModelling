import torch
from deform_cnn.deform_cnn import DeformableConv2d
...
class Encoder_mod(torch.nn.Module):
    def __init__(self, num_classes=102, num_filters=32, input_size=(16, 1,40,40)):
        ...
        # Encoder Layer1
        self.encoder_layer1_conv = DeformableConv2d(1, num_filters, 5,1, padding='same')# ,padding=(3, 3))
       ...
       # Encoder Layer2
        self.encoder_layer2_conv = DeformableConv2d(num_filters, 2 * num_filters, 5,1, padding='same')# ,padding=(3, 3))
        ...
        # Encoder Layer3
        self.encoder_layer3_conv = DeformableConv2d(2 * num_filters, 3*num_filters, 5,1, padding='same') # ,padding=(3, 3))
...
class Decoder_mod(torch.nn.Module):
    def __init__(self, num_classes=102, num_filters=32, input_size=(16, 1,40,40)):
        ...
        self.decoder_layer1_conv =  DeformableConv2d(3 * num_filters, 3*num_filters, 5,1, padding='same')
        ...
        self.decoder_layer2_conv =DeformableConv2d(3*num_filters, 2*num_filters, 5,1, padding='same')
        self.decoder_layer2_batch_norm = torch.nn.BatchNorm2d(2*num_filters, eps=1e-3, momentum=0.99)
        self.decoder_layer3_conv =DeformableConv2d(2*num_filters , num_filters, 5,1, padding='same')
        self.decoder_layer3_batch_norm = torch.nn.BatchNorm2d(num_filters, eps=1e-3, momentum=0.99)
        self.activation = torch.nn.ReLU()
        self.activation1 = torch.nn.ReLU()
        self.decoder_layer4_conv =  DeformableConv2d(num_filters,1 , 5,1, padding='same')
